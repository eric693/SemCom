...
def fixed_point_to_float_sign_5_5_6(x_fixed, int_bits, frac_bits):
    # 轉換為定點數的 Tensor
    x_fixed = x_fixed.view(-1, int_bits + frac_bits + 1)

    # 轉換符號位元
    sign = torch.where(torch.sum(x_fixed[:, :1], dim=1) > 0, -1, 1)

    # 轉換整數位元
    int_part = x_fixed[:, 1:1+int_bits]
    int_part = torch.where(torch.sum(int_part, dim=1) > 0, 1, 0)

    # 轉換小數位元
    frac_part = x_fixed[:, 1+int_bits:1+int_bits+frac_bits]
...
embeds_shared = Embeds(vocab_size=38, num_hidden=512).to(device)
...
def do_test(input_data, encoder, decoder, normlize_layer, channel, len_batch):
    ...
    with torch.no_grad():
        ...
        print('Channel noise :')
        flipped = flip_bits(pred1, n=1)
        print(flipped)

        # 將二進制表示轉換為浮點數
        pred8 = fixed_point_to_float_sign_5_5_6(flipped, int_bits=1, frac_bits=6)
