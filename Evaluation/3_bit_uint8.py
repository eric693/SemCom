...
embeds_shared = Embeds(vocab_size=38, num_hidden=256).to(device)
encoder = LSTMEncoder(channel_dim=channel_dim, embedds=embeds_shared).to(device)
decoder = LSTMDecoder(channel_dim=channel_dim, embedds=embeds_shared, vocab_size = 38).to(device)
    ...
    with open('log.txt', 'w') as log_file:
        for input_str in SemanticRL_example:
            ...
            for ckpt_dir in [args.ckpt_pathRL]:#, args.ckpt_pathRL
                ...
                # encoder.load_state_dict(torch.load('/home/eric/research/SemanticRL/ckpt_AWGN_RL/3bit_2/encoder_epoch201.pth', map_location='cpu'))
                # decoder.load_state_dict(torch.load('/home/eric/research/SemanticRL/ckpt_AWGN_RL/3bit_2/decoder_epoch201.pth', map_location='cpu'))
                # embeds_shared.load_state_dict(torch.load('/home/eric/research/SemanticRL/ckpt_AWGN_RL/3bit_2/embeds_shared_epoch201.pth',  map_location='cpu'))
