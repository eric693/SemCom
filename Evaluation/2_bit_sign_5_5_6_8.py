...
embeds_shared = Embeds(vocab_size=20, num_hidden=512).to(device)
encoder = LSTMEncoder(channel_dim=channel_dim, embedds=embeds_shared).to(device)
decoder = LSTMDecoder(channel_dim=channel_dim, embedds=embeds_shared, vocab_size = 20).to(device)
...
if __name__ == "__main__":
    ...
    dict_train = pickle.load(open('./2.train_dict.pkl', 'rb'))
    ...
    with open('log.txt', 'w') as log_file:
        for input_str in SemanticRL_example:
            ...
            for ckpt_dir in [args.ckpt_pathRL]:#, args.ckpt_pathRL
                ...
                encoder.load_state_dict(torch.load('/home/eric/research/SemanticRL/ckpt_AWGN_RL/2bit/encoder_epoch201.pth', map_location='cpu'))
                decoder.load_state_dict(torch.load('/home/eric/research/SemanticRL/ckpt_AWGN_RL/2bit/decoder_epoch201.pth', map_location='cpu'))
                embeds_shared.load_state_dict(torch.load('/home/eric/research/SemanticRL/ckpt_AWGN_RL/2bit/embeds_shared_epoch201.pth',  map_location='cpu'))
