...
from model import LSTMEncoder, LSTMDecoder, Embeds, TransformerEncoder, TransformerDecoder
...
embeds_shared = Embeds(vocab_size=123, num_hidden=512).to(device)
encoder = TransformerEncoder(channel_dim=channel_dim, embedds=embeds_shared).to(device)
decoder = TransformerDecoder(channel_dim=channel_dim, embedds=embeds_shared, vocab_size = 123).to(device)
...
def do_test(input_data, encoder, decoder, normlize_layer, channel, len_batch):
    ...
    with torch.no_grad():
        ...
        output = decoder.sample_max_batch(output, len_batch) #None
    return output

SemanticRL_example = [  'rockets launch into cosmos',
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_pathRL", type=str, default='./ckpt_AWGN_RL')
