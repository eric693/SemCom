...
def fixed_point_to_float_sign_5_5_6(x_fixed, int_bits, frac_bits):
    # 轉換為定點數的 Tensor
    x_fixed = x_fixed.view(-1, int_bits + frac_bits + 29)

    # 轉換符號位元
    sign = torch.where(torch.sum(x_fixed[:, :29], dim=1) > 14, -1, 1)

    # 轉換整數位元
    int_part = x_fixed[:, 29:29+int_bits]
    int_part = torch.where(torch.sum(int_part, dim=1) > 14, 1, 0)

    # 轉換小數位元
    frac_part = x_fixed[:, 29+int_bits:29+int_bits+frac_bits]
...
embeds_shared = Embeds(vocab_size=16, num_hidden=512).to(device)
encoder = LSTMEncoder(channel_dim=channel_dim, embedds=embeds_shared).to(device)
decoder = LSTMDecoder(channel_dim=channel_dim, embedds=embeds_shared, vocab_size = 16).to(device)
...
def do_test(input_data, encoder, decoder, normlize_layer, channel, len_batch):
    ...
    with torch.no_grad():
        ...
        print('Channel noise :')
        flipped = flip_bits(pred1, n=10)
        print(flipped)

        # 將二進制表示轉換為浮點數
        pred8 = fixed_point_to_float_sign_5_5_6(flipped, int_bits=29, frac_bits=6)
...
if __name__ == "__main__":
    ...
    dict_train = pickle.load(open('./2bit_2.train_dict.pkl', 'rb'))
    ...
    with open('log.txt', 'w') as log_file:
        for input_str in SemanticRL_example:
            ...
            for ckpt_dir in [args.ckpt_pathRL]:#, args.ckpt_pathRL
                ...
                encoder.load_state_dict(torch.load('/home/eric/research/SemanticRL/ckpt_AWGN_RL/2bit_2/encoder_epoch201.pth', map_location='cpu'))
                decoder.load_state_dict(torch.load('/home/eric/research/SemanticRL/ckpt_AWGN_RL/2bit_2/decoder_epoch201.pth', map_location='cpu'))
                embeds_shared.load_state_dict(torch.load('/home/eric/research/SemanticRL/ckpt_AWGN_RL/2bit_2/embeds_shared_epoch201.pth',  map_location='cpu'))
